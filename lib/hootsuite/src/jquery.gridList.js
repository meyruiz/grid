...
(function (factory) {
    ...
    DraggableGridList.prototype = {
        ...
        _onDrag: function (event, ui) {
            ...
            var canvasHeight = gridWidth * 24 * 12;
            var canvasWidth = gridHeight * 24;
            ...
            if (ui.position.left < 0)
                ui.position.left = 0;
            if (ui.position.left + (this._cellWidth * (this._getItemWidth(item) / 24)) > canvasWidth) // multiply per item's width / 20 (20 is the grid's size in px)
                ui.position.left = canvasWidth - (this._cellWidth * (this._getItemWidth(item) / 24));
            if (ui.position.top < 0)
                ui.position.top = 0;
            if (ui.position.top + (this._cellHeight * (this._getItemHeight(item) / 24)) > canvasHeight)
                ui.position.top = canvasHeight - (this._cellHeight * (this._getItemHeight(item) / 24));
        ...
        _calculateCellSize: function () {
            if (this.options.direction === "horizontal") {
                ...
                // Prev like this because it was calculated based on this.options.lanes instead of width/height. Now changed to support different sizes.
                // this._cellWidth = this._cellHeight * this.options.widthHeightRatio; 
            } else {
                this._cellWidth = Math.floor(20 / zoomScale);
                this._cellHeight = 20 / zoomScale / this.options.widthHeightRatio / 12;
                //this.$element.height() / this.options.height / zoomScale / this.options.widthHeightRatio;
            }
            if (this.options.heightToFontSizeRatio) {
                this._fontSize = this._cellHeight / this.options.heightToFontSizeRatio * 2;
            }
        },
        ...
        _getItemHeight: function(item) {
            return item.h * this._cellHeight; // turn inches to ft
        },
