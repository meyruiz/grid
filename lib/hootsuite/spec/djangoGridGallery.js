...
function openDialogConfig(el) {
    ...
    gridData["dialog"].find("div input.lenFT").val(Math.floor(length / 12));
    gridData["dialog"].find("div input.lenIN").val(length % 12);
    gridData["dialog"].find("div input.width").val(width);
...
function applyDialogConfig() {
    var lengthFT = parseInt(gridData["dialog"].find("div input.lenFT").val());
    var lengthIN = parseInt(gridData["dialog"].find("div input.lenIN").val());
    var width = parseInt(gridData["dialog"].find("div input.width").val());
...
function initializeDialog() {
    gridData["dialog"] = $("#belt-cut-settings");
...
$(function () {
    gridData["DemoGrid"] = {
        ...
        buildElements: function (items) {
            ...
            for (i = 0; i < items.length; i++) {
                ...
                // Need to hide controls too for cuts for other transactions
        ...
        offcut: function (type, id) {
            ...
            if (type == "horizontal") {
                if (x == 0) {
                    var posX = x + w;
                    var width = data['width'] - w;
                    h *= 12; // convert inches to ft
                    gridData["DemoGrid"].addItem(posX, y, h, width, "Offcut");
                    let percentage = getPercentageOffcut(width, "horizontal");
                    return {x: posX, y: y, h: h, w: width, percOffcut: percentage};
                }
                ...
                if (x + w == data['width']) {
                    ...
                    let percentage = getPercentageOffcut(width, "horizontal");
                    return {x: 0, y: y, h: h, w: width, percOffcut: percentage};
                }

            } else {
                var posY = y + h;
                var height = (gridData["DemoGrid"].height / 12) - (y + h);
                posY *= 12; // position from in to ft
                height *= 12;
                gridData["DemoGrid"].addItem(x, posY, height, w, "Offcut");
                let percentage = getPercentageOffcut(height / 12, "vertical");
                return {x: x, y: posY, h: height, w: w, percOffcut: percentage};
            }
        },
    ...
    gridData["DemoGrid"].currentSize = gridWidth;
    gridData["DemoGrid"].width = gridWidth;
    gridData["DemoGrid"].height = gridHeight * 12;
    ...
    $('.add-horizontal-offcut').click(function (e) {
        ...
        setCostInformation(this.dataset.id, offcutData);
